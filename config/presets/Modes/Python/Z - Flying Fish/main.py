...
def setup(screen, etc):
    global w1, h1, moverGroup, myETC, myScreen, startRect, bg, dragRect
    w1 = screen.get_width()
    ...
    dragRect = pygame.Rect(0,2*h1/3,w1,h1/3)
...
def draw(screen, etc):
    ...
    pygame.draw.rect(myScreen, (43, 101, 236), dragRect)
    for b in moverGroup:
...
class Mover:
    ...
    def drag(self, c, knob3):

        scaledR = self.r * (1 + knob3)

        diff = 2*h1 / 3 - (self.position.y +  scaledR)
        if (diff <= scaledR / 2):
            # self.velocity *= .95
            drag = self.velocity
            drag.normalize()
