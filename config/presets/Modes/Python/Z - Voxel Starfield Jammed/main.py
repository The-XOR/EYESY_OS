...
COLORS2 = [orangered, darkorange, coral, sandybrown, seashell, palegoldenrod, lightcyan, honeydew, lightsteelblue,
           cornflowerblue, royalblue, powderblue]
...
COLORS6 = [mediumspringgreen, palegreen, chartreuse, lime, aqua, darkturquoise, deepskyblue, dodgerblue, blue, indigo,
           mediumblue, slateblue]
COLORS7 = [mediumspringgreen, palegreen, chartreuse, lime, aqua, darkturquoise, deepskyblue, dodgerblue, blue, indigo,
           mediumblue, slateblue, red, firebrick, crimson, indianred, tomato, darkred, palevioletred, fuchsia, orchid,
           salmon, maroon]
COLORS = COLORS7
...
class Star:
    ...
    def update(self, knob2, knob3):
        ...
        self.pos3d = self.get_pos3d(300 * knob3 + 32) if self.pos3d.z < 1 else self.pos3d
    ...
    def draw(self, audioIn):
        s = self.size
        audio_size = (audioIn[self.sound_ind % 100]) / 500
        if (audio_size < 2):
            audio_size = 0
        if (-s < self.screen_pos.x < WIDTH + s) and (-s < self.screen_pos.y < HEIGHT + s):
            pg.draw.rect(self.screen, COLORS[self.color_ind % len(COLORS)],
                         (self.screen_pos.x, self.screen_pos.y, self.size + audio_size, self.size + audio_size))
...
def draw(screen, etc):
    global SPINFACTOR, COLORS  # PITCH, YAW
    SPINFACTOR = -3 + etc.knob1 * 6
    if (etc.knob1 == 0):
        SPINFACTOR = 0
    # PITCH = 1000* etc.knob2
    # YAW = 1000 * etc.knob3
    # Z_DISTANCE = 170  #150 * etc.knob5 +20
    ...
    if color_choice == 0:
        ...
        COLORS = COLORS7
    etc.starfield.run(etc.knob2 * 2, etc.knob2, etc.audio_in)
    if (16 >= 64 * etc.knob5 + 16):  # 32
        screen2 = screen
    else:
        screen2 = pixelate_surface(screen, int(64 * etc.knob5) + 16)

    # Trails
    # veil = pg.Surface((1280,720))
    ...
    if etc.knob3 == 0:
        ...
        blurFactor = 16 + 80 * etc.knob3
        screen2 = blurSurf(screen2, blurFactor)
