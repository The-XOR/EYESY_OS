...
def draw(screen, etc) :
    ...
    lastScreenLFO.step = int(etc.knob2*50) #LFO rate of change
    lastScreenSize = lastScreenLFO.update()
    thingX = int(xr-(lastScreenSize))#int(xr-(etc.knob2*200))
    thingY = int(yr-(lastScreenSize*0.5625))#int(yr-(etc.knob2*200))
...
def seg(screen, etc, i) :
    ...
    if etc.knob1 < 0.33 : 
        linewidth = int(((abs(shapey)*3.5)*etc.xres)/(lines-75)+1)
        ballSize = 0 #no balls shown
        shapeLFO.step = etc.knob1*etc.knob1 #LFO rate of change
    
    if etc.knob1 >= 0.33 and etc.knob1 < 0.66 :
        linewidth = 0 #no lines shown
        ballSize = int((((0.66-abs(shapey*2))*3)*etc.xres)/(lines-75)+1)
        shapeLFO.step = (etc.knob1-0.33)*(etc.knob1-0.33) #LFO rate of change
    
    if etc.knob1 >= 0.66 :
        linewidth = int(((abs(1-shapey)-0.66)*1.5)*etc.xres/(lines-75))
        ballSize = int((((abs(shapey*3-0.66))*3)*etc.xres)/(lines-75))
        shapeLFO.step = (etc.knob1-0.66)*(etc.knob1-0.66) #LFO rate of change
        
    
    sel = etc.knob4*8 #select the colors with Knob 4
