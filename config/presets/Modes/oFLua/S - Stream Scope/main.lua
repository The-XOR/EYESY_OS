...
modeTitle = "S - Stream Scope"      -- name the mode
...
function draw()
    ...
    k2Slide.target = ( (1-knob2)*(w*2) ) + w/2.1                 --zoom
    ...
    if( freezeCount > 1 ) then
        for i = 1, freezeCount-1 do
            ...
            oscMesh( h/globYAmt, 0, (h/globYAmt)/2, 0, w/globXAmt, h/200, glm.vec3(0,1,0), 0, lineDef, theTable[stream] )
            of.popMatrix()
        end
    end
