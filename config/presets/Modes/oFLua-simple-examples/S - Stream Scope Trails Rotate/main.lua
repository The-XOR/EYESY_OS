-- S - Stream Scope Trails Rotate
require("eyesy")                                -- include the eyesy library
modeTitle = "S - Stream Scope Trails Rotate"    -- name the mode
...
function setup()
    ...
    oscLineWidth = h/100                -- how wide the scope line is
    
    -- define img class
    theImg = of.Image()
    
    -- frame buffer 
    fbo = of.Fbo()                      -- frame buffer
    fbo:allocate( w, h )                -- allocate width, and height
    
    fbo:beginFbo()                      -- clear the buffer
        of.clear(0,0,0,0)
    fbo:endFbo()
...
function update()
    fbo:beginFbo()
        ...
        --knob Stuff
        speed = math.pow( knob1 * 20, 2)                -- speed
        k2Slide.target = ( knob2 * 180  )             -- zoom
                          
        rotate = k2Slide.update()
        ...
        of.setColor( bg, math.pow(1-knob3, 2)*255 )       -- color is background, control opacity with knob3
        of.drawRectangle( glm.vec3(0,0,0), w, h )       -- draw overlay rectangle
        ...
        oscMesh( h/globYAmt, 0, (h/globYAmt)/2, zoomer, w/globXAmt, oscLineWidth, glm.vec3(0,1,0), 0, lineDef, wowTab )
        ...
        if( freezeCount > 1 ) then
            for i = 1, freezeCount-1 do
                stream = (globFrameCount-i) % (globXAmt*globYAmt)
                stream = stream + 1
                j = math.floor( (i) / 8 )
                itr = i % 8 
                ...
                of.pushMatrix()
                    ...
                    oscMesh( h/globYAmt, 0, (h/globYAmt)/2, zoomer, w/globXAmt,oscLineWidth, glm.vec3(0,1,0), 0, lineDef, theTable[stream] )
                of.popMatrix()
