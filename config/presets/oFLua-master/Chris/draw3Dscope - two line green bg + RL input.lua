...
----------------------------------------------------
function draw3DScopeL(a, b, amplitude, axis)
...
function draw3DScopeR(a, b, amplitude, axis)
    local stepx = (b.x - a.x) / 256
    local stepy = (b.y - a.y) / 256
    local stepz = (b.z - a.z) / 256
    of.beginShape()
    for i=1,256 do
        if axis == 1 then
            of.vertex(a.x + stepx*i + inR[i]*amplitude, a.y + stepy*i, a.z + stepz*i)
        end
        if axis == 2 then
            of.vertex(a.x + stepx*i, a.y + stepy*i + inR[i]*amplitude, a.z + stepz*i)
        end
        if axis == 3 then
            of.vertex(a.x + stepx*i, a.y + stepy*i, a.z + stepz*i + inR[i]*amplitude)
        end
    end
    of.endShape()
end
