...
knob3 = .5 --tower2 distance + fog strength
knob4 = .5 --tower color
knob5 = .5 --bg color
...
function draw()
    count = count+1
    Fog = glm.ceil(knob5*59+323)
    of.setBackgroundColorHex(colortable[Fog])
    
    local Red = glm.mod(count,255)
    local Green = glm.mod(count/2,255)
    local Blue = glm.mod(count/3,255)
...
function shadow(Red, Green, Blue, alpha)
    of.translate(-knob3*100,0,-knob3*Width)
    boxrow()
    of.setColor(255,255,255,255*knob3)
    of.drawPlane(0,0,0,10*Width,10*Height)
end
    
function boxrow()
    local boxes = 10+knob2*50
    local space = Height/boxes
    local range = glm.ceil(knob4*255+1)
    for i=1,boxes do
        R = inL[i]*200+100*(1-knob2)+1
        x = knob1*Width--
        y = i*(space) - space/2
        z = 0
        of.setHexColor(colortable[i+300+range])
        of.drawBox(x,y,z,R)
    end
end
