...
function draw()
    ...
    ColorVal = glm.ceil(knob4*150+1)
    FScolor = of.Color.fromHex(colortable[ColorVal])
    Black = of.Color(0,0,0)
    of.background(of.Color.fromHex(colortable[glm.ceil(knob5*50)+1]))
    ...
    boing = inL[1]*1000
    rotspeed = knob3*100+1
    
    of.pushMatrix()
        of.translate(Width / 2, Height / 2)
        of.rotateYDeg(count/2)
        of.rotateXDeg(count/3)
        of.rotateZDeg(count/4)
...
function cool(alpha)
    of.translate(boing, -boing, boing/2)
    of.setColor(FScolor, alpha)
    drawBox1()
end
    
function drawBox1()
    local R = (knob2)*Height
    local Cubes = knob3*23+1
    for i=1,Cubes do
        local x = R*i/6 
        local y = Height/12
        local v = glm.vec3(x, y, i*20)
        of.drawBox(v, inR[i]*10+i*30,inR[i]*10+i*30,inR[i]*10+i*30)
    end
end
----------------------------------------------------
function exit()
