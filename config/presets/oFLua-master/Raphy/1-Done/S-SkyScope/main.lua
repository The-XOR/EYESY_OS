...
function draw()
    ...
    of.translate(0,knob3*Height,0)
        of.pushMatrix()
            of.rotateXDeg(120*knob3)
            drawBars()
        of.popMatrix()
end

function drawBars()
    local Bars = 250*knob2 + 6
    for i=1,Bars do
        sliding = count*i/3
        x=sliding%Width
        x1=x-5
        ...
        y1 = y+inL[i]*1000
        of.translate(0,0,1)
        of.setHexColor(colortable[i+glm.ceil(knob4*10)])
        of.drawLine(x,y,x1,y1)
    end
end
