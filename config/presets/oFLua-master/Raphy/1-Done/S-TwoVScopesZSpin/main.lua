...
function draw()
    ...
    bgcolor = glm.ceil(knob5*116+1)
    of.setBackgroundColorHex(cooltable[bgcolor])
    color = 559+count%41
    of.setHexColor(cooltable[color])
    
    count = count+1
    
    of.pushMatrix()
        of.translate(0,0,knob3*Width-Width)
        of.translate(Width/2,Height/2,0)
        of.pushMatrix()
            of.translate(0,0,-Width)
            vertScopeL()
        of.popMatrix()
    of.popMatrix()
    
    of.pushMatrix()
        of.translate(0,0,knob3*Width)
        of.translate(Width/2,Height/2,0)
        of.pushMatrix()
            of.translate(0,0,-Width)
            vertScopeR()
...
function vertScopeL()
    of.setLineWidth(5*knob3+1)
    of.rotateXDeg(knob4*count*20)
    of.beginShape()
    local sliderL = knob1*Width-Width/2
    for i=1,256 do
       of.vertex( inL[i]*800+sliderL,(i-1)*Height/255-Height/2)
    end
    of.endShape()
end

function vertScopeR()
    of.setLineWidth(5*knob3+5)
    of.rotateXDeg(knob4*count*10)
    of.beginShape()
    local sliderR = -(knob2*Width-Width/2)
    for i=1,256 do
       of.vertex( inR[i]*800+sliderR,(i-1)*Height/255-Height/2)
    end
    of.endShape()
end
