...
function draw()
    ...
    local Red = 255*knob4
    local Green = 200*knob4
    local Blue = 30*knob4
    
    for j=1,10 do
        of.translate(j,j,0)
        of.setHexColor(colortable[280+j])
        drawBars()
    end
...
function drawBars()
    local ybounce = glm.sin(Bcount*knob3/20)
    local bars = 25*knob1+1
    local lineW = 10
    local Pan = Width*(glm.sin(count/12))/2+Width/2
    of.setLineWidth(lineW)
    of.rotateXDeg(knob4*45)
    of.translate(0,0,-knob4*Width)
    
    for i=1,bars do
        x=count
        x1=x+inL[i*2]*1000+10
        y = Height - ((Height*i/25)+ybounce*Height)-10
        of.drawLine(x+Pan*knob2,y,x1+Pan*knob2,y)
    end
    
    for i=1,bars do
        x=Width-count-50
        x1=x+inR[i*2]*1000+10
        y = (Height*i/25)+ybounce*Height-40
        of.drawLine(x-Pan*knob2,y,x1-Pan*knob2,y)
    end
