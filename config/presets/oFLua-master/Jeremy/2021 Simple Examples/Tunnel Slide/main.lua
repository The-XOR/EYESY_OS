...
modeTitle = "Tunnel Slide"                -- name the mode
...
function setup() 
    ...
    for i = 1, 10 do
        slideTable[i] = slider()
        theCircles[i] = circle( 0, 0, h, i/10)
    end  
...
function draw()
    ...
    for i = 1, 10 do
        sound = (inL[i] * h) + h/2
        slideTable[i].target = sound
        sound2 = slideTable[i].update()
        --theCircles[i].xWarp = k3
        theCircles[i].yPos = sound2
...
function circle( x, y, maxSize, modd )
    ...
    function self.makeCircle()
       ...
       if( self.count > 1) then
        ...
        of.pushMatrix()
        of.translate(self.xPos, self.yPos, (modder*self.maxSize)-(self.maxSize/2) )
        of.rotateXDeg(90)
...
function slider()
    ...
    function self.update()
        if( math.abs(self.target - self.current) > 0.1 ) then
            ...
            --set step
            --local space = math.abs(self.target - self.current )
            self.step =  1 --space/ 2
