-- S - Sound Planet Lines
require("eyesy")                    -- include the eyesy library
modeTitle = "S - Sound Planet Lines"       -- name the mode
...
function draw()
    ...
    of.rotateYDeg( theSlideY:update() )
    ...
    for i = 0, circleRez do
        ...
        local pointMax = glm.vec3( xIter*(h*2), yIter*(h*2), 0 )
        local pointMin = glm.vec3( xIter*h, yIter*(h), 0 )
