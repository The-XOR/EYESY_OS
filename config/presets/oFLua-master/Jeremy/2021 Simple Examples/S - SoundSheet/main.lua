...
modeTitle = "S - SoundSheet"        -- name the mode
...
function draw()
    ...
    rotateX = knob2*360
    -- set the plane attributes
    thePlane:set(w, globDepth, 20, maxAmt, of.PRIMITIVE_TRIANGLES )
    thePlane:setPosition( glm.vec3( w/2,(globDepth/2), 0) )
    
    modFg:setHsb( fg:getHue(), fg:getSaturation(), fg:getBrightness() - 20 )
    
	theLight:setAmbientColor( of.FloatColor( modFg.r/255, modFg.g/255, modFg.b/255) ) -- make the ambient color white
	theLight:setAreaLight(20, 20)
    ...
    of.pushMatrix()
    of.translate(0,h/2)
    of.rotateXDeg( rotateX )
    
    
    
    theMesh = thePlane:getMesh()
    ...
    of.pushMatrix()
    of.translate( -w/2, 0, -(globDepth/2) )
    of.rotateXDeg(90)
    theLight:setPosition( 0, h, -globDepth/2)           -- set light position, change x
...
function lines( amt, modd )
    ...
    function self.makeLine()
        ...
        if(modder < self.modCompare ) then
            ...
            for i = 1, 20 do
                self.freezeTable[i] = inL[i*5] * h/4
            end
        end
        ...
        for i = 0, 19 do
            iter = i + ( math.floor(modder*maxAmt)*20 )
            theMesh:setVertex( iter, glm.vec3( (w/19)*i, self.freezeTable[i+1], modder * globDepth) )
        end
