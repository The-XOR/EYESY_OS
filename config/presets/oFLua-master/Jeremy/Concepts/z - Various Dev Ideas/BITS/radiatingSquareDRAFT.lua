...
hiHlf = Height / 2
wiHlf = Width / 2
sqmover = LFOtri( -hiHlf, hiHlf, 0.01 )
adjust1 = LFOtri( -50, 50, 0.01 )
adjust2 = LFOtri( -100, 100, 0.01)
x490 = relX(490)
y210 = relY(210)
x190 = relX(190)
y510 = relY(510)
...
function line(i)
    -- functions to abreviate x,y calcs
    function ajustW( x, ad, i )
        o2 = (ad * i) + relX(x)
        o1 = (o2 / Width) % Width
        return o1
    end
    function ajustH( x, ad, i )
        o2 = (ad * i) + relY(x)
        o1 = (o2 / Height) % Height
        return o1
    end
        
    auD = inL[ (i%100) + 1 ]    -- audio buffer, tables in lua start with 1
    ...
    elseif( knob4 <= 0.09 ) then
        ...
        of.setHexColor(colortable[glm.ceil(knob4*130+1)])   -- knob4 color
    end
    -- different draw modes controlled by knob4
    if( i < 25 ) then
        x = ajustW( x490, aj1, i )          -- calculate x
        y = ajustH( y210, aj2, i*12 )          -- calculate y
        xA = x - auD                        -- add the audio
        of.drawLine( x, y, xA, y - angle)   -- draw the line
    
    elseif( i >= 25 and i < 50 ) then
        x = ajustW( x190, aj2, i*12 )        -- calculate x
        y = ajustH( y510, aj1, i )           -- calculate y
        yA = y + auD                        -- add the audio
        of.drawLine( x, y, x+angle, yA )    -- draw the line   
    
    elseif( i >= 50 and i < 75 ) then
        x = ajustW( x790, aj1, i )        -- calculate x
        y = ajustH( y1110, aj2, i*12 )      -- calculate y
        xA = x + auD                        -- add the audio
        of.drawLine( x, y, xA, y+angle )    -- draw the line  
    
    elseif( i >= 75 and i < 100 ) then
        x = ajustW( x1690, aj2, i*12 )      -- calculate x
        y = ajustH( y210, aj1, i )          -- calculate y
        yA = y - auD                        -- add the audio
        of.drawLine( x, y, x-angle, yA)     -- draw the line

    elseif( i == 1 ) then
        x = ajustW( x490, aj2, i )      -- calculate x
        y = ajustH( y210, aj1, i )          -- calculate y
        yA = y - auD                        -- add the audio
        of.drawLine( x, y, x-angle, yA )     -- draw the line
    end
...
function draw()
    FScol = glm.ceil( (knob5*56) + 200 )                -- Background color knob5
    of.setBackgroundColorHex(colortable[FScol])         -- Background color stuff
    time = of.getElapsedTimeMillis() * 0.0008           -- Global time

    width = math.floor( knob2 * relX(15) ) + 1
    of.setLineWidth( width )
    aj1 = adjust1.update()
    adjust1.step = knob1 / 50
    aj2 = adjust2.update()
    adjust2.step = knob1 + .001
