...
-----------------------------------------------------
function drawSpiral( radius, length, xPos, yPos, relSize, rotate, tubeRadius )
    yLength = length / 100          -- divide the length into 100 parts                  
    largerRad = radius * relSize    -- get the max distance
    rWidth = ribbon 
    for i = 0, 7 do
        iter = i / 7
        xVersion = math.sin( iter * 6.28 ) * tubeRadius
        yVersion = math.cos( iter * 6.28 ) * tubeRadius
        for i = 1, 500 do               -- 100 for loop
            audio =  inL[ (i%100)+1 ]   -- get the audio
            ...
            myMesh:addVertex( glm.vec3( x+xVersion,y+yVersion,z) )
        end
    end
    for i = 0, 498 do
        for j = 0, 7 do
            ...
            second = ((j+1) % 7) * 500
            ------ first triangle
            myMesh:addIndex( i + first )
            myMesh:addIndex( (i+1) + first )
            myMesh:addIndex( i + second )
            ------ second triangle
            myMesh:addIndex( i + second )
            myMesh:addIndex( (i+1) + first )
            myMesh:addIndex( (i+1) + second )
        end
    end
...
function draw()
    ...
    myCam:beginCamera()                             -- begin camera
        myLight:enable()                            -- start the light
            ...
            of.fill()
            drawSpiral( spiralW, spiralH, w2, h2, 4, count, 60 )
