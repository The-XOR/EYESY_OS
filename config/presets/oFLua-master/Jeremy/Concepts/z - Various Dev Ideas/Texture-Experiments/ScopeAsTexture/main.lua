modeTitle = "Sphere with Scope Texture"
...
light = of.Light()
myImg = of.Image()
myImg:load( "/home/../sdcard/Modes/oFLua/ScopeAsTexture/Images/1.png" )
myImg:getTexture():setTextureWrap(of.REPEAT, of.REPEAT)
myFbo = of.Fbo()
myFbo:allocate( Width, Height )
myFbo:getTexture():setTextureWrap(of.REPEAT, of.REPEAT)
mySphere = of.SpherePrimitive()

function oscLines( xStart, xSize, yPos, lineAmt )
    of.noFill()
    xSpace = xSize / lineAmt
    of.beginShape()
    for i = 0, lineAmt do
        sound = inL[ (i%100) + 1 ] * heightHalf
        x = i * xSpace 
        of.vertex( x + xStart, yPos + sound )
    end
    of.endShape()
end
...
function setup()
    ...
    light:lookAt( glm.vec3(widthHalf,0,0) ) 
end
-- oscillascope function
...
function draw()
    ...
    knob4Color = colortable[ glm.ceil( knob4 * 130) + 1 ]
    
    --myImg:grabScreen( 200, 300, 400, 400 )
    myFbo:beginFbo()
    oscLines( widthHalf/2, widthHalf, knob2*Height, 50 )
    myFbo:endFbo()
    
    
    mySphere:setRadius( knob1*400 )
    mySphere:setResolution( 100 )
    of.setColor( 255,255,255, 255 )
    mySphere:setPosition( widthHalf, heightHalf, 0)
