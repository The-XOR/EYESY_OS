...
----------------------- Color stuff
    ...
    for j = 0, lines do
        ...
        of.drawLine( 0, space, Width, space )
    end
    
    for m = 0, lines do
        x = math.floor( m * spaceW ) + 2
        y = 0
        ...
        of.setColor( tabi[1], tabi[2], tabi[3] )
        of.drawLine( x, y, x, y + auDio )
        if( recsize >= 1) then
            of.drawRectangle( x-(recsize/2), y+auDio, recsize, recsize )
        end
    end
    for i = 0, lines do
        x = math.floor(i*spaceW) + 1
        y = Height - recsize
        imod = (i+50) % 100
        ...
        of.setColor( tabi[1], tabi[2], tabi[3] )
        of.drawLine( x, y, x, y - (auDio*-1) )
        if( recsize >= 1 ) then
            of.fill()
            of.drawRectangle( x-(recsize/2) + 1, y+auDio, recsize, recsize )
        end
