...
if(three <= 0) then             -- depending on ratio if three is to small make it 1
    three = 1
end
-- DRAW LOOP ------------------
...
function draw()
   ...
   ----------------------- Color stuff
    ...
    width = (knob3 * hund) + 3
    sel = math.floor(knob4 * 6) + 1

    of.pushMatrix()
    of.noFill()                         -- dont fill the squares
    of.setLineWidth(2)
    for i = 0, 6 do                     -- for loops to make the squares
        for j = 0, 9 do
            rgbTab = {255,255,255}                 -- table for square color 
            x = (j * x8) + (x8/2)
            y = (i * y5) - (y5/2)
